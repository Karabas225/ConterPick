import { storeFirst, storeRun } from "./app-store";
...
/**
 * The first administrator is intentionally stable for the self-hosted MVP.
 * COUNTERPICK_ADMIN_EMAILS can still add more administrators at deploy time.
 */
export const DEFAULT_ADMIN_EMAIL = "alebaba4565@gmail.com";
...
type UserRow = { id: string; email: string | null; phone: string | null; display_name: string; role: string };

function userFromRow(row: UserRow): AppUser {
  return { id: row.id, email: row.email ?? null, phone: row.phone ?? null, displayName: row.display_name, role: row.role };
}
...
export async function getCurrentUser(request: Request): Promise<AppUser | null> {
  ...
  const row = await storeFirst<UserRow>(
    "SELECT u.id, u.email, u.phone, u.display_name, u.role FROM auth_sessions s INNER JOIN users u ON u.id = s.user_id WHERE s.token_hash = ? AND s.expires_at > ? LIMIT 1",
    [tokenHash, Date.now()],
  );
  if (!row) return null;
...
export function configuredAdminEmails() {
  const configured = (runtimeEnv("COUNTERPICK_ADMIN_EMAILS") ?? "")
    .split(",")
...
export async function createSession(userId: string) {
  ...
  await storeRun("INSERT INTO auth_sessions (token_hash, user_id, created_at, expires_at) VALUES (?, ?, ?, ?)", [await digest(token), userId, Date.now(), Date.now() + SESSION_TTL_MS]);
  return token;
}
...
export async function deleteSession(request: Request) {
  const token = cookieValue(request, SESSION_COOKIE);
  if (!token) return;
  await storeRun("DELETE FROM auth_sessions WHERE token_hash = ?", [await digest(token)]);
}
