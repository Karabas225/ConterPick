...
export default function Home() {
  ...
  return <main className="site-shell" data-catalog-version={catalogVersion}><nav className="topbar"><a className="brand" href="#top" aria-label="CounterPick — наверх"><span className="brand-mark" /><span>COUNTER<span>PICK</span></span></a><div className="nav-links"><a href="#draft">Драфт</a><a href="#results">Рекомендации</a><a href="#support">Поддержка</a>{user?.role === "admin" && <button type="button" className="nav-button" onClick={() => setAdminOpen((value) => !value)}>Админ</button>}</div><div className="topbar-actions"><div className={`status-chip ${liveUpdate?.status === "error" ? "error" : ""}`}><span />PATCH {liveUpdate?.patch ?? "…"}</div><button type="button" className="account-button" onClick={() => setAuthOpen((value) => !value)}>{user ? user.displayName : "Войти"}</button>{authOpen && <AuthCard user={user} onUser={setUser} onClose={() => setAuthOpen(false)} />}</div></nav>
    ...
    <footer><div className="brand"><span className="brand-mark" /><span>COUNTER<span>PICK</span></span></div><span>Драфт на шаг впереди.</span><span className="footer-note">Dota 2 data intelligence · by Karabas</span></footer>
  </main>;
}
