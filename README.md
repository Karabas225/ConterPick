...
Для Windows-хоста используйте `deploy/windows/counterpick-production.cmd` как действие Планировщика задач с триггером «При входе в систему». Скрипт запускает production-сервер на `0.0.0.0:3000`, пересобирает проект перед стартом и автоматически повторяет запуск после аварийного завершения. Не запускайте production через временный SSH-сеанс: Windows завершает дочерний процесс после закрытия сеанса.
